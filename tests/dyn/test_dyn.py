...
def test_discretization_methods():
    ...
    for dt in dts:
        ...
        Ad_true = np.array([[1, dt],
                            [0, 1]])
        Bd_true = np.array([[0.5*dt**2],
                            [dt]])
...
def test_linear_accel():
    ...
    x_traj[:, 0] = np.array([0., 20., 0.])
    y_traj = np.zeros((dyn.p, H+1))
    y_traj[:, 0] = np.array([0., 20.])
    ...
    for k in range(H):
        ...
        x_traj_true[2, k+1] = (1 - dt/tau)*x_traj_true[2, k] + dt/tau*u_traj[0, k]
...
def test_linear_vel():
    ...
    for k in range(H):
        x_traj_true[0, k+1] = x_traj_true[0, k] + dt * x_traj_true[1, k]
        x_traj_true[1, k+1] = (1 - dt/tau)*x_traj_true[1, k] + dt/tau*u_traj[0, k]
