...
DISCRETIZATION_METHODS = ['forward euler',
                          'piecewise constant input']

DYNAMICS_OPTIONS = ['linear acceleration',
                    'linear velocity']


def pw_const_input_discretization(A: np.ndarray, 
                                  B: np.ndarray, 
...
def forward_euler_discretization(A: np.ndarray,
                                 B: np.ndarray,
                                 dt: float) -> Tuple[np.ndarray, np.ndarray]:
    """
