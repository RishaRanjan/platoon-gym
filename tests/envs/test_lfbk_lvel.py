...
def test_platoon_env_vel_dyn_lfbk_ctrl():
    ...
    vehs = [Vehicle(dyns[0], veh_args, position=0, velocity=20.0)]
    vehs += [
        Vehicle(dyns[i], veh_args, position=-i * d_des - i, velocity=20.0)
        for i in range(1, n_vehicles)
    ]
    render_mode = "plot"
    env = gym.make(
        "platoon_gym-v0", vehicles=vehs, virtual_leader=vl, env_args=env_args, render_mode=render_mode
    )
    obs, _ = env.reset()
    actions = []
